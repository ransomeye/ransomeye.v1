// Path and File Name : /home/ransomeye/rebuild/ransomeye_dpi_probe/src/identity.rs
...
impl Identity {
    pub fn load_or_create(config: &Config) -> Result<Self, IdentityError> {
        let identity_dir = Path::new(&config.buffer_dir).parent()
            .unwrap_or(Path::new("/var/lib/ransomeye/dpi_probe"));
    ...
    fn create(key_path: &Path, id_path: &Path) -> Result<Self, IdentityError> {
        ...
        let keypair = RsaKeyPair::from_pkcs8(&keypair_bytes)
            .map_err(|e| IdentityError::KeyGenerationFailed(format!("{}", e)))?;
        
        // Generate producer ID from public key hash
        // Extract public key from PKCS8
        let public_key_der = &keypair_bytes[24..]; // Approximate - PKCS8 structure
        let mut hasher = Sha256::new();
        hasher.update(&keypair_bytes);
        let hash = hasher.finalize();
        let producer_id = format!("dpi_probe_{}", hex::encode(&hash[..16]));
    ...
    fn load(key_path: &Path, id_path: &Path) -> Result<Self, IdentityError> {
        ...
        let keypair = RsaKeyPair::from_pkcs8(&keypair_bytes)
            .map_err(|e| IdentityError::LoadFailed(format!("Failed to parse key: {}", e)))?;
    ...
    pub fn keypair(&self) -> RsaKeyPair {
        // Clone keypair by recreating from bytes
